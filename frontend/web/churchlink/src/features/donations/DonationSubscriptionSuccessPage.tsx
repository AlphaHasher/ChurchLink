...
import { CheckCircle, XCircle, ArrowLeft, ExternalLink, Loader2, CreditCard } from "lucide-react";
...
const DonationSubscriptionSuccessPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (!subscriptionId) {
            ...
            setState({
                phase: "warning",
                message:
                    localize("We returned from PayPal but didn't receive a subscription id. If your approval succeeded, you'll get a confirmation email shortly."),
                token,
                baToken,
            });
            return;
                ...
                <Card className="max-w-md w-full">
                    <CardContent className="pt-6">
                        <div className="text-center">
                            ...
                            <h2 className="text-xl font-semibold text-gray-900 mb-2">{localize("Finalizing Subscription")}</h2>
                            <p className="text-gray-600">{localize("One moment while we wrap things up…")}</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
                ...
                <Card className="max-w-lg w-full">
                    <CardContent className="pt-6">
                        <div className="text-center">
                            ...
                            <h2 className="text-xl font-semibold text-gray-900 mb-4">{localize("Something went wrong")}</h2>
    ...
    const InfoGrid = ({
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <Label htmlFor="subId">{localize("Subscription ID")}</Label>
                <Input id="subId" readOnly value={subId || "—"} />
            </div>
            <div>
                <Label htmlFor="token">{localize("Token")}</Label>
                <Input id="token" readOnly value={tk || "—"} />
            </div>
            <div>
                <Label htmlFor="batoken">{localize("Billing Agreement Token")}</Label>
            ...
            <div className="min-h-screen bg-gray-50 py-8">
                <div className="max-w-3xl mx-auto px-4">
                    <Card className="border-yellow-200">
                        <CardHeader>
                            <CardTitle className="text-yellow-700">{localize("Approval Received")}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            <Alert className="border-yellow-300 text-yellow-800 bg-yellow-50">
                                <AlertDescription>
                                    {localize("We couldn't detect a subscription id in the callback. If the approval succeeded, PayPal will email you a confirmation and your account will reflect the new subscription shortly.")}
                                </AlertDescription>
                            </Alert>
                ...
                <Card className="border-green-200">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2 text-green-700">
                            ...
                            {localize("Subscription Approved")}
                        </CardTitle>
                    </CardHeader>
                    ...
                    <CardContent className="space-y-6">
                        <div className="flex flex-wrap gap-2">
                            ...
                            <Badge variant="secondary">{localize("Recurring Donation")}</Badge>
                        </div>
                        ...
                        <div className="text-sm text-muted-foreground">
                            {localize("Your subscription may take a moment to appear in your donor history. You’ll receive a receipt from PayPal for each successful charge. If you need to change or cancel, you can manage it in PayPal or through your account once it syncs.")}
                        </div>
