...
//   if (role !== "admin") return <p>Access Denied</p>; // Restrict to admins only
  ...
  const [selectedTimezone, setSelectedTimezone] = useState('America/Los_Angeles'); // Default PST
  ...
  const handleRemoveScheduledNotification = async (id: string) => {
    ...
    try {
      await fetch(`/api/scheduled-notification/${id}`, { method: "DELETE" });
      fetchScheduledNotifications();
    } catch {}
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6">
        ...
        {envOverride && (
          <div className="mb-2 p-2 bg-yellow-100 border border-yellow-400 rounded text-yellow-800">
            These settings are managed in the backend <code>.env</code> file.<br />
            To edit, update your <code>.env</code> or disable override to allow editing from the frontend.
          </div>
        )}
        ...
        <div className="mb-2">
          <label className="block text-sm font-medium text-gray-700 mb-1">YouTube Primary Channel ID</label>
          <input
            type="text"
            placeholder="Enter YouTube Primary Channel ID"
        ...
        <div className="mb-2">
          <label className="block text-sm font-medium text-gray-700 mb-1">YouTube Public Domain</label>
          <input
            type="text"
            placeholder="Enter YouTube Public Domain"
            value={youtubePublicDomain}
            onChange={(e) => setYoutubePublicDomain(e.target.value)}
            className="border p-2 w-full mb-2"
            disabled={envOverride}
          />
        </div>
        ...
        <div className="mb-2">
          <label className="block text-sm font-medium text-gray-700 mb-1">YouTube Time Zone</label>
          <select
          ...
          >
            ...
            <option disabled>Need more timezone? Contact DEV</option>
          </select>
        </div>
        <button
        ...
        >
          {envOverride ? "Edit in .env" : loading ? "Saving..." : "Save YouTube Channel"}
        </button>
      </div>
      ...
      {/* Notification List */}
      <h2 className="text-xl font-semibold mt-6">Notification List</h2>
