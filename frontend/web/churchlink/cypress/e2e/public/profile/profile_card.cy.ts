...
describe('Profile / Edit Profile', () => {
    ...
    it('shows an error for invalid first name', () => {
        ...
        cy.contains('button', 'Save changes').click().then(() => {
            expect(alertStub).to.have.been.called;
            const msg = alertStub.getCall(0).args[0] as string;
    ...
    it('shows an error when birthday is missing', () => {
        openEditDialog();
    ...
    it('shows an error when birthday is under 13 years old', () => {
        openEditDialog();

        cy.get('#self-firstName').clear().type('Cypress');
        cy.get('#self-lastName').clear().type('Tester');

        const underage = getUnderageDob();
    ...
    it('successfully updates name, DOB, and gender with valid inputs', () => {
        openEditDialog();

        const newFirst = 'CypressValid';
        const newLast = 'UserValid';
        const adult = getAdultDob();
        ...
        cy.contains('dt', 'Account name')
            .parent()
            .within(() => {
                cy.contains(`${newFirst} ${newLast}`).should('be.visible');
            });
