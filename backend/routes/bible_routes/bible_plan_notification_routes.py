...
from models.bible_plan_tracker import update_user_bible_plan_notifications as update_plan_notifications
...
bible_notification_router = APIRouter(
    prefix="/bible-plan-notifications", 
    tags=["Bible Plan Notifications"]
)
...
async def update_user_bible_plan_notifications(
    request: Request,
    plan_id: str,
    notification_request: NotificationTimeRequest
):
    ...
    try:
        ...
        updated = await update_plan_notifications(
            uid=uid,
            plan_id=plan_id,
...
async def update_device_bible_plan_preference(
    ...
    try:
        result = await update_bible_plan_notification_preference(
            ...
            enabled=device_request.enabled
        )
...
async def send_immediate_notification(
    ...
    try:
        result = await BiblePlanNotificationManager.send_immediate_bible_plan_notification(
            uid=uid,
...
async def update_device_bible_plan_preference(request: DeviceNotificationPreferenceRequest):
    """Update Bible plan notification preference for a specific device token"""
    try:
        result = await update_bible_plan_notification_preference(
            token=request.token, 
    ...
    try:
        ...
        if result.get("success"):
            return {
                ...
                "bible_plan_reminders_enabled": result.get("bible_plan_reminders_enabled"),
...
async def get_all_device_preferences_endpoint():
    ...
    try:
        from helpers.BiblePlanNotificationHelper import get_all_bible_plan_notification_preferences
