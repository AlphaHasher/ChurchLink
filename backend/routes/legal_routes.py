...
user_legal_router = APIRouter(prefix="/legal", tags=["legal"])
admin_legal_router = APIRouter(prefix="/admin/legal", tags=["legal-admin"])

# Public
@user_legal_router.get("/{slug}", response_model=LegalPagePublicOut)
...
@admin_legal_router.get("", response_model=List[LegalPageAdminOut])
async def list_pages():
    return await admin_list_pages()

@admin_legal_router.get("/{slug}", response_model=LegalPageAdminOut)
...
@admin_legal_router.put("/{slug}", response_model=LegalPageAdminOut)
...
@admin_legal_router.delete("/{slug}")
