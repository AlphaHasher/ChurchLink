...
FormTxnStatus = Literal["created", "captured", "partially_refunded", "fully_refunded", "failed"]
...
def _normalize_refunds(raw_list: Any) -> List[TransactionRefund]:
    """
    Tolerant parser for existing refund arrays on form_transactions.
...
async def record_form_refund(
    *,
    paypal_capture_id: str,
    refund: TransactionRefund,
) -> Optional[dict]:
    """
    Append a refund entry to a form transaction and update its status.

    Idempotent per refund_id: if a refund with the same refund_id already exists,
    the list is left as-is, but status/updated_at are refreshed.
    """
    capture_id = (paypal_capture_id or "").strip()
    if not capture_id:
        return None

    doc = await DB.db.form_transactions.find_one({"paypal_capture_id": capture_id})
    if not doc:
        return None
    ...
    return await DB.db.form_transactions.find_one_and_update(
        {"_id": doc["_id"]},
        update,
        return_document=True,
    )
