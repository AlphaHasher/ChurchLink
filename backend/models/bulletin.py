...
class BulletinBase(BaseModel):
    ...
    image_id: Optional[str] = Field(
        None, description="Media library image ID (24-char ObjectId)"
    )
...
class BulletinFeedOut(BaseModel):
    ...
    )  # List[ServiceBulletinOut] to avoid circular import
...
async def create_bulletin(bulletin: BulletinCreate) -> Optional[BulletinOut]:
    ...
    payload["publish_date"] = strip_timezone_for_mongo(
        normalize_to_local_midnight(bulletin.publish_date)
    )

    # Also normalize expire_at if provided
    if bulletin.expire_at:
        payload["expire_at"] = strip_timezone_for_mongo(
            normalize_to_local_midnight(bulletin.expire_at)
        )
    ...
    if not isinstance(order_value, int) or order_value <= 0:
        ...
        try:
            if DB.db is not None:
                max_order_doc = await DB.db["bulletins"].find_one(
    ...
    try:
        ...
        print(
            f"Error: Bulletin with headline '{bulletin.headline}' already exists (duplicate key)"
        )
        return None
    except Exception as exc:
...
async def get_bulletin_by_headline(
    headline: str, exclude_id: Optional[str] = None
) -> Optional[BulletinOut]:
    """
...
async def update_bulletin(bulletin_id: str, updates: BulletinUpdate) -> bool:
    ...
    if "publish_date" in update_payload:
        update_payload["publish_date"] = strip_timezone_for_mongo(
            normalize_to_local_midnight(update_payload["publish_date"])
        )

    # Normalize expire_at to local midnight if provided
    if "expire_at" in update_payload and update_payload["expire_at"] is not None:
        update_payload["expire_at"] = strip_timezone_for_mongo(
            normalize_to_local_midnight(update_payload["expire_at"])
        )
...
async def list_bulletins(
    *,
    ...
    if upcoming_only:
        ...
        query.setdefault("publish_date", {})["$lte"] = strip_timezone_for_mongo(
            local_now
        )
